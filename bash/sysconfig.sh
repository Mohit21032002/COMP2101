...
echo  "system information not properly displaying it should go through" >&2
}
...
function error-exit {
echo "system information caused some error" >&2   
}

#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {
echo "
Usage: $O [-h] -- Program to show system information
     -h         show help information
     --host     Report only on the host name
     --domain   Report only on domain name
     --cpu      Report regaring printer
     --disk     Report only on system disk information
     --os       Report only on operating system
     --memory   Report only on memory information
     --ipconfig Report on IP information"      
}
